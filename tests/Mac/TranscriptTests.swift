...
@main struct TranscriptTests {
    static func main() {
        ...
        precondition(ListeningIndicator.state(held: true, connected: false, muted: false) == .preparing)
        precondition(ListeningIndicator.state(held: true, connected: true, muted: true) == .preparing)
        precondition(ListeningIndicator.state(held: true, connected: true, muted: false) == .listening)
        precondition(ListeningIndicator.state(held: false, connected: true, muted: false) == .idle, "Release stops the indicator immediately")
        precondition(display.opacity(for: QuickTranscript(), busy: true, indicator: .preparing, at: 0) == 1, "Show the face before the first transcript")
        precondition(display.opacity(for: QuickTranscript(), busy: true, indicator: .listening, at: 0) == 1)
        precondition(display.opacity(for: QuickTranscript(), busy: false, at: 0) == 0, "Empty muted overlay hides")
