...
First-time setup needs ChatGPT sign-in, Microphone and Screen Recording permissions for Little Guy, and the Computer Use plugin enabled in Codex. Native controls use Codex Computer Use’s own macOS permissions. **Microphone audio is captured only while you hold the shortcut.** Releasing mutes new input immediately; speech already captured during startup finishes sending through your ChatGPT connection. End voice, explicit dismissal, sleep, or Quit closes the connection and clears queued audio. Escape cancels the current request and prepares a fresh muted connection; Escape in another app leaves the connection alone when the popup is hidden. Typed questions and manual image attachments remain available. See [the Mac guide](docs/MACOS.md) for tested behavior and supported controls.
